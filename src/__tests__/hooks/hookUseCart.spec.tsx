...
describe("Hook useCard", () => {
  ...
  it("Teste de remover produto", () => {
    ...
    act(() => {
      result.current.removeProduct(idProduct);
    });
  ...
  it("Teste para atualizar a quantidade de um produto do estoque", async () => {
    const idProduct = 2;
   ...
   apiMock.onGet(`stock/${idProduct}`).reply(200, {
    ...
    act(() => {
      result.current.updateProductAmount({ amount: 4, productId: idProduct });
    });
